...
def nmpc_controller(ref_trajectory):
    ...
    cost += 150.0 * (target[1] - x[4, N])**2  # final y position error
    cost += 150.0 * (target[0] - x[3, N])**2 # final x position error

    ## Stage costs (at k)
    for k in range(N):
        cost += 50 * (target[1] - x[4, k])**2 # y position error
        cost += 50 * (target[0] - x[3, k])**2 # x position error

        # cost += 20.0 * x[5, k]**2  # phi_k -> 0
        # cost -= 20.0 * (x[0, k])**2    # Ux_k -> inf
        cost += 2 * u[0, k]**2  # F_xk
        cost += 2 * u[1, k]**2   # gamma_k
